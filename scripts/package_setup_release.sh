...
#
# Writes: dist/dino-crisis-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix dino-crisis \
  --exe-name Dino_Crisis_Recompiled \
  --display-name "Dino Crisis Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Dino Crisis (USA) (Rev 1).cue" \
